...
def selector(options):
  print("\n Select a gcard by number (1-{0}) from the above list: ".format(len(options)-1))
  selection = input()
  return selection
...
def select_gcard(args):
  ...
  while (selection not in np.arange(1,len(content))) or not (isinstance(selection,int)):
    print("\n Selection not in valid range, try again, or hit ctrl+c to exit")
    selection = selector(content)
